...
// Precache assets generated by Vite (filtering out any image assets so 422 errors never crash SW)
const precacheList = (self.__WB_MANIFEST || []).filter((entry) => {
  const url = typeof entry === 'string' ? entry : (entry?.url || '');
  return !url.includes('.png') && !url.includes('.jpg') && !url.includes('.jpeg') && !url.includes('.webp') && !url.includes('.ico');
});
