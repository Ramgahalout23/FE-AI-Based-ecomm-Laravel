...
export default function SettingsAdminPage() {
  ...
  // Database auto-cleanup scheduler state (Hybrid: 24h Cron + Admin Trigger)
  ...
  useEffect(() => {
    ...
    if (tab === 'system') {
      fetchHealth();
      adminAPI.getCleanupStatus().then((res) => {
  ...
  const TAB_SETTING_KEYS = {
    ...
    'payments': [
      ...
      'codEnabled', 'codInstructions', 'codBlockedPincodes', 'codMaxRefusals', 'codRefusalAction',
      'partialPaymentEnabled', 'partialPaymentMinAmount', 'partialPaymentPercentage', 'partialPaymentInstructions',
      'orderOtpEnabled', 'orderOtpExpiry',
    ],
    'tax': [
  ...
  const handleRunCleanup = async (apply = false) => {
    if (apply) {
      if (!(await confirm({
        ...
        message: 'This will permanently remove expired logs, visitor tracking events, and read notifications older than their retention windows (in non-blocking batches of 1,000). Active and unread data is strictly preserved.',
      ...
      {tab === 'payments' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="detail-panel">
            <div className="detail-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <h3>🚫 COD Restrictions</h3>
                <span style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Prevent COD abuse</span>
              </div>
            </div>
            <div className="form-grid" style={{ marginTop: '1rem' }}>
              <div className="form-group form-full">
            ...
            <div className="form-actions" style={{ marginTop: '1rem' }}><button className="btn-dark btn-sm" onClick={handleSaveSettings} disabled={loading}>{loading ? 'Saving...' : 'Save COD Restrictions'}</button></div>
          </div>
