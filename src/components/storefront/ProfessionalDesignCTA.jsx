  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative w-full sm:max-w-lg bg-[#111] rounded-t-2xl sm:rounded-2xl overflow-hidden shadow-2xl border border-white/10 max-h-[90vh] overflow-y-auto"
      >
      ...
      <div ref={scrollRef} onScroll={updateArrows} className="flex gap-3 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-2 -mx-4 px-4">
        {designs.map((design, i) => (
          <motion.button
            ...
            className="gallery-card relative min-w-[160px] w-[160px] sm:min-w-0 snap-start rounded-xl overflow-hidden bg-gray-800 border border-white/10 shrink-0 group cursor-pointer"
          >
...
export default function ProfessionalDesignCTA() {
  ...
  return (
    <>
      <section className="relative py-12 sm:py-16 lg:py-20 overflow-hidden bg-[#0d0d0d]">
        <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-stretch gap-8 sm:gap-10 lg:gap-14">
            {/* ── LEFT: CTA Content ── */}
            <motion.div
            ...
            >
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <span className="text-white/40 text-[10px] font-bold uppercase tracking-[0.24em]">Custom Design</span>
              </div>

              <h2 className="text-3xl sm:text-4xl md:text-5xl font-display font-extrabold tracking-tight leading-[1.08]">
              ...
              <p className="text-white/45 text-sm sm:text-base mt-4 leading-relaxed max-w-lg">
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-2.5 mt-6 sm:mt-7">
                {features.map((f, i) => (
                  <motion.div
                    key={f.label}
                    ...
                    className="flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border border-white/10 hover:border-white/25 transition-colors duration-300"
                  >
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/70 shrink-0">
                      {f.icon}
                    </div>
                    <div className="min-w-0">
                      <span className="block text-white/85 text-[11px] sm:text-xs font-semibold leading-tight truncate">{f.label}</span>
                      <span className="hidden sm:block text-white/30 text-[9px] font-medium">{f.desc}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="mt-7 sm:mt-8">
                <button
                  ...
                  className="group relative inline-flex items-center gap-2.5 sm:gap-3 px-7 sm:px-9 py-3.5 sm:py-4 rounded-full bg-white text-[#0d0d0d] text-xs sm:text-sm font-bold tracking-wide transition-all duration-300 hover:-translate-y-0.5 active:translate-y-0 hover:shadow-[0_0_30px_rgba(255,255,255,0.15)] w-full sm:w-auto justify-center"
                >
                ...
                <p className="text-white/25 text-[9px] sm:text-[10px] mt-2.5 font-medium tracking-wide text-center sm:text-left">
            ...
            >
              <div className="relative">
                <div className="flex sm:hidden items-center justify-center gap-2 mb-3">
                  ...
                  <span className="text-white/25 text-[7px] font-bold uppercase tracking-[0.3em]">Inspiration Gallery</span>
                ...
                <div className="hidden sm:grid sm:grid-cols-2 gap-3 p-5 border border-white/10 rounded-2xl bg-white/[0.02]">
                  {SAMPLE_DESIGNS.map((design, i) => (
                    <motion.button
                      ...
                      className="group relative aspect-[3/4] rounded-xl overflow-hidden bg-gray-800 border border-white/10 hover:border-white/25 transition-all duration-500 cursor-pointer"
                    >
                      <img src={design.image} alt={design.title} loading={i < 2 ? 'eager' : 'lazy'} className="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-105"
