          ...
          <div className="flex items-center justify-between h-16 gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
              ...
              <button
                ...
                className="p-2 text-white/80 hover:text-primary transition-colors rounded-lg hover:bg-white/10"
              >
                <Search size={22} />
              </button>
              ...
              <Link 
                to="/wishlist" 
                className="relative p-2 text-white/80 hover:text-primary transition-colors rounded-lg hover:bg-white/10 hidden sm:flex"
              >
                <Heart size={22} />
              </Link>
              ...
              {(isAuthenticated || isAdmin) ? (
                ...
                >
                  <button className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-white/80 hover:text-primary hover:bg-white/10 transition-colors">
                    <User size={20} />
                    <span className="hidden lg:inline">{isAdmin ? 'Admin' : (user?.firstName || 'Account')}</span>
                  </button>
                  ...
                  <AnimatePresence>
                    {showAccount && (
                      ...
                      >
                        ...
                        <div className="py-1">
                          {isAdmin ? (
                            <Link to="/admin" className="block px-4 py-2.5 text-sm text-text-secondary hover:bg-surface hover:text-primary transition-colors">Dashboard</Link>
                          ) : (
                            <>
                              <Link to="/profile" className="block px-4 py-2.5 text-sm text-text-secondary hover:bg-surface hover:text-primary transition-colors">My Profile</Link>
                              <Link to="/orders" className="block px-4 py-2.5 text-sm text-text-secondary hover:bg-surface hover:text-primary transition-colors">My Orders</Link>
                              <Link to="/addresses" className="block px-4 py-2.5 text-sm text-text-secondary hover:bg-surface hover:text-primary transition-colors">Addresses</Link>
                            </>
                          )}
                        </div>
              ...
              <button
                ...
                className="relative flex items-center gap-2 p-2 text-white/80 hover:text-primary transition-colors rounded-lg hover:bg-white/10"
              >
              ...
              <button
                ...
                className="lg:hidden p-2 text-white/80 hover:text-primary transition-colors hover:bg-white/10 rounded-lg"
              >
            ...
            >
              <div className="px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom,0px))] space-y-4"
                ...
                <div className="flex flex-col gap-1 border-t border-white/10 pt-3">
                  ...
                  <Link to="/" className="px-4 py-3.5 rounded-xl text-sm font-medium text-white/80 hover:bg-white/10">Home</Link>
                  <Link to="/products" className="px-4 py-3.5 rounded-xl text-sm font-medium text-white/80 hover:bg-white/10">All T-Shirts</Link>
                  {/* Custom pages */}
                  {customPages.slice(0, 5).map((page) => (
                    <Link
                      key={page.slug}
                  ...
                  <Link to="/wishlist" className="px-4 py-3.5 rounded-xl text-sm font-medium text-white/80 hover:bg-white/10">Wishlist</Link>
                  {!isAuthenticated && !isAdmin && (
                    <Link to="/login" className="mt-2 text-center py-3 rounded-xl text-sm font-bold bg-primary text-white">Sign In</Link>
                  )}
                  {(isAuthenticated || isAdmin) && (
                    <button onClick={async () => { await logout(); localStorage.removeItem('adminToken'); navigate('/'); }} className="mt-2 text-center py-3 rounded-xl text-sm font-bold bg-red-600 text-white">Logout</button>
                  )}
                </div>
              </div>
            </motion.div>
